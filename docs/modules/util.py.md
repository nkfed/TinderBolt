...
- Особливості: перед відправкою текст проходить `.encode('utf16', errors='surrogatepass').decode('utf16')` для коректної обробки емодзі/сурогатів.
...
Надсилає HTML‑повідомлення (`ParseMode.HTML`) з тією ж UTF‑16 обробкою для емодзі.
...
- Параметри:
  - `text: str` — текст (Markdown); буде пропущений через UTF‑16 обробку.
...
2. Емодзі/сурогати:
   - UTF‑16 з `surrogatepass` допомагає коректно пройти емодзі; якщо виникають артефакти — перевіряйте клієнт та вхідні дані.
